#pragma once

#include <DefaultBundle.hpp>

class Camera : public System
{
public:
    void start() override
    {
        m_cameraEntity = world().createEntity(CCamera{}, CTransform{glm::vec3(0.0f, 0.0f, 5.0f)});

        m_input = services().get<Input>();
        m_window = services().get<Window>();
    }

    void update(double deltaTime) override
    {
        float dt = static_cast<float>(deltaTime);
        if (auto [entity, cCam, transform] = world().fetchAt<CCamera, CTransform>(0); entity != 0 && m_mouseCaptured)
        {
            transform->rotation.y += -m_input->getMouseDelta().x * 0.2f;
            transform->rotation.x += -m_input->getMouseDelta().y * 0.2f;

            transform->rotation.x = glm::clamp(transform->rotation.x, -89.0f, 89.0f);

            float yaw = glm::radians(transform->rotation.y);
            float pitch = glm::radians(transform->rotation.x);

            glm::vec3 forward;
            forward.x = -cos(pitch) * sin(yaw);
            forward.y = sin(pitch);
            forward.z = -cos(pitch) * cos(yaw);

            forward = glm::normalize(forward);

            glm::vec3 right = glm::normalize(glm::cross(forward, glm::vec3(0, 1, 0)));

            glm::vec2 direction(0.0f);

            if (m_input->isKeyDown(Key::A))
                direction.x = -1;
            if (m_input->isKeyDown(Key::D))
                direction.x = 1;
            if (m_input->isKeyDown(Key::W))
                direction.y = 1;
            if (m_input->isKeyDown(Key::S))
                direction.y = -1;
            direction = glm::length(direction) != 0 ? glm::normalize(direction) : direction;

            float speed = 5.0f * dt;
            if (m_input->isKeyDown(Key::LShift))
                speed *= 2.0f;

            transform->position += forward * direction.y * speed;
            transform->position += right * direction.x * speed;
        }

        if (m_input->isKeyPressed(Key::Escape))
        {
            m_window->captureMouse(false);
            m_mouseCaptured = false;
        }
        if (m_input->isKeyPressed(Key::LAlt))
        {
            m_window->captureMouse(true);
            m_mouseCaptured = true;
        }
    }

private:
    Entity m_cameraEntity;
    Input *m_input;
    Window *m_window;
    bool m_mouseCaptured = true;
};